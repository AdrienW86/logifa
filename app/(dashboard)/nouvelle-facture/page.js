...
function FormulaireFacture() {
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen bg-slate-50 overflow-x-hidden">
      
      {/* HEADER STICKY CENTRÉ */}
      <div className="sticky top-0 z-30 w-full flex justify-center bg-slate-50/80 backdrop-blur-md px-4 py-4 border-b border-slate-200/50">
        <div className="max-w-4xl w-full flex flex-col sm:flex-row justify-between items-center bg-white p-3 rounded-2xl shadow-sm border border-slate-200 gap-4">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <button onClick={() => setShowPreview(true)} className="flex-1 flex items-center justify-center gap-2 bg-slate-50 text-slate-600 px-5 py-2 rounded-xl text-xs font-black hover:bg-slate-100 transition-all border border-slate-200">
              <Eye size={14}/> APERÇU
            </button>
            <button onClick={handleSave} disabled={isSaving} className="flex-1 flex items-center justify-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-xl text-xs font-black hover:bg-blue-700 disabled:opacity-50 transition-all shadow-md shadow-blue-100">
              <Save size={14}/> {isSaving ? "..." : "ENREGISTRER"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl w-full p-4 md:p-10 space-y-8 pb-32">
        
        {/* ÉMETTEUR / CLIENT */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <section className="p-6 bg-white rounded-3xl border border-slate-200 shadow-sm">
            <h2 className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest">Émetteur</h2>
            <input type="text" className="w-full p-2 text-sm font-bold border-b border-slate-100 outline-none mb-2 bg-transparent" value={monEntreprise.nom} onChange={(e) => setMonEntreprise({...monEntreprise, nom: e.target.value})} />
            <input type="text" className="w-full p-2 text-xs text-slate-500 outline-none bg-transparent" value={monEntreprise.adresse} onChange={(e) => setMonEntreprise({...monEntreprise, adresse: e.target.value})} />
          </section>
          
          <section className="p-6 bg-white rounded-3xl border border-slate-200 shadow-sm">
            <h2 className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest">Client</h2>
            <select className="w-full p-2 bg-slate-50 border border-slate-100 rounded-lg text-xs font-bold mb-2 outline-none" value={selectedClientId} onChange={(e) => handleClientSelection(e.target.value)}>
              <option value="nouveau">+ Nouveau client</option>
              {dbClients.map(c => <option key={c.id} value={c.id}>{c.nom_entreprise}</option>)}
            </select>
            <input type="text" placeholder="Nom de l'entreprise *" className="w-full p-2 text-xs border-b border-slate-50 outline-none bg-transparent" value={clientInfo.nom_entreprise} onChange={(e) => setClientInfo({...clientInfo, nom_entreprise: e.target.value})} />
          </section>
        </div>
        ...
        <section className="space-y-4">
                ...
                <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-3">
                  {items.map((item, index) => (
                    <Draggable key={item.id} draggableId={item.id} index={index}>
                      {(p, snapshot) => (
                        <div ref={p.innerRef} {...p.draggableProps} className={`flex items-center gap-2 ${snapshot.isDragging ? "z-50" : ""}`}>
                          {/* POIGNÉE DE DRAG (Visible partout, fix mobile) */}
                          <div {...p.dragHandleProps} className="text-slate-300 p-2 touch-none active:text-blue-500 transition-colors">
                            <GripVertical size={20}/>
                          </div>

                          <div className={`flex-1 p-4 rounded-2xl border transition-all ${snapshot.isDragging ? "shadow-2xl scale-[1.02]" : "shadow-sm"} ${item.type === 'section' ? 'bg-slate-800 border-slate-800' : 'bg-white border-slate-200'}`}>
                            {item.type === 'section' ? (
                              ...
                              <div className="space-y-3">
                                <div className="flex justify-between gap-2">
                                  <input className="font-bold text-sm outline-none w-full bg-transparent" placeholder="Désignation..." value={item.description} onChange={(e) => setItems(items.map(i => i.id === item.id ? {...i, description: e.target.value} : i))} />
                                  <div className="flex items-center bg-slate-50 px-2 py-1 rounded-lg">
                                    <span className="text-[9px] font-black text-slate-400 mr-1 uppercase">TVA</span>
                                ...
                                <div className="flex flex-wrap items-center gap-4 border-t border-slate-50 pt-3">
                                  ...
                                  <select className="text-[10px] font-black uppercase text-slate-400 bg-transparent outline-none" value={item.unite} onChange={(e) => setItems(items.map(i => i.id === item.id ? {...i, unite: e.target.value} : i))}>
                                    {UNITES.map(u => <option key={u} value={u}>{u}</option>)}
                                  </select>
                                  <div className="flex-1 text-right">
                                    <input type="number" className="w-24 text-right font-black text-sm outline-none bg-transparent" value={item.prixHT} onChange={(e) => setItems(items.map(i => i.id === item.id ? {...i, prixHT: e.target.value} : i))} />
                                    <span className="ml-1 text-[10px] font-bold text-slate-400 uppercase">€ HT</span>
                                  </div>
                                </div>
                              </div>
                            )}
                          ...
                          <button onClick={() => setItems(items.filter(i => i.id !== item.id))} className="text-slate-300 hover:text-red-500 p-2 transition-colors">
                            <Trash2 size={18}/>
                          </button>
                        </div>
                      )}
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => setItems([...items, { id: crypto.randomUUID(), type: 'item', description: '', quantite: 1, unite: 'unité', prixHT: 0, tva: 20 }])} className="flex-1 flex items-center justify-center gap-2 text-[10px] font-black px-5 py-4 bg-white border border-slate-200 rounded-2xl uppercase hover:bg-slate-50 transition-all shadow-sm"><Plus size={14}/> Ajouter Article</button>
            <button onClick={() => setItems([...items, { id: crypto.randomUUID(), type: 'section', titre: 'NOUVELLE SECTION' }])} className="flex-1 flex items-center justify-center gap-2 text-[10px] font-black px-5 py-4 bg-white border border-slate-200 rounded-2xl uppercase hover:bg-slate-50 transition-all shadow-sm"><Plus size={14}/> Ajouter Section</button>
          </div>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div className="p-6 bg-white border border-slate-200 rounded-3xl shadow-sm space-y-4">
              <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-tight">Exonération TVA (Auto-entrepreneur)</span>
              ...
              {docType === 'facture' && (
                <div className="flex justify-between items-center border-t border-slate-50 pt-4">
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-tight">Montant d'acompte déjà versé</span>
                  <input type="number" className="w-24 text-right font-black text-blue-600 text-sm outline-none bg-transparent" value={montantAcompte} onChange={(e) => setMontantAcompte(parseFloat(e.target.value) || 0)} />
                </div>
              )}
           </div>
           
           <div className="p-8 bg-slate-900 rounded-3xl shadow-xl text-white flex flex-col justify-center relative overflow-hidden group">
              <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><FileText size={80}/></div>
              <p className="text-[10px] font-black text-slate-400 uppercase mb-1 relative z-10 tracking-widest">NET À PAYER TTC</p>
              <p className="text-4xl font-black relative z-10">{ (totalTTC - (docType === 'facture' ? montantAcompte : 0)).toFixed(2) } €</p>
           </div>
        </div>
      ...
      {showPreview && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/95 backdrop-blur-md p-4">
          <button onClick={() => setShowPreview(false)} className="absolute top-6 right-6 z-[110] bg-white/10 hover:bg-white/20 p-3 rounded-full text-white transition-all hover:rotate-90">
            <X size={24}/>
          </button>

          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-[110] flex gap-4 w-full px-4 justify-center">
             <button onClick={() => setShowPreview(false)} className="bg-white/10 text-white px-6 py-4 rounded-2xl text-[10px] font-black backdrop-blur-md border border-white/10 hover:bg-white/20 transition-all uppercase tracking-widest">Fermer</button>
             <PDFDownloadLink 
                ...
                fileName={`${numDoc}.pdf`}
                className="flex items-center gap-2 bg-blue-600 text-white px-8 py-4 rounded-2xl text-[10px] font-black shadow-2xl hover:bg-blue-700 transition-all hover:scale-105 uppercase tracking-widest"
              >
                {() => <><FileText size={16}/> Télécharger PDF</>}
              </PDFDownloadLink>
          </div>

          <div className="w-full h-full overflow-y-auto flex justify-center py-10 lg:py-20 scrollbar-hide">
            <div className="origin-top transform scale-[0.35] sm:scale-[0.55] md:scale-[0.75] lg:scale-[0.9] xl:scale-100 transition-transform duration-500 shadow-2xl mb-32">
              <div className="w-[21cm] bg-white p-[1.5cm] flex flex-col min-h-[29.7cm] rounded-sm text-slate-900">
                <div className="flex justify-between items-start mb-16">
                  <div>
                    <div className="w-16 h-16 bg-slate-50 rounded-2xl mb-4 flex items-center justify-center overflow-hidden border border-slate-100">
                      {user?.imageUrl ? <img src={user.imageUrl} className="w-full h-full object-cover" /> : <FileText className="text-slate-200"/>}
                    </div>
                ...
                <table className="w-full mb-12">
                  ...
                  <tbody>
                    {items.map((item) => (
                      item.type === 'section' ? (
                        <tr key={item.id} className="bg-slate-50"><td colSpan="4" className="py-3 px-4 font-black text-[10px] text-slate-500 uppercase tracking-widest">{item.titre}</td></tr>
                      ) : (
                ...
                <div className="mt-auto flex justify-end">
                  <div className="w-72 space-y-3 border-t-2 border-slate-100 pt-8">
                    ...
                    {montantAcompte > 0 && <div className="flex justify-between text-[11px] font-bold text-blue-600 uppercase"><span>Acompte</span><span>- {montantAcompte.toFixed(2)} €</span></div>}
                    ...
                    {isExonere && <p className="text-[9px] text-slate-400 italic text-right mt-6 font-bold">TVA non applicable, art. 293 B du CGI</p>}
                  </div>
                </div>
              </div>
            </div>
